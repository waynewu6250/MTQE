...
class DecoderLayer(nn.Module):
    ...
    def forward(self, x, mask, memory):
        ...
        out = self.self_attn(x, memory, memory, None)
        x = out + self.dropout(out)
...
class TransformerEncoder(nn.Module):
    def __init__(self, device, hidden_size, attn_head, num_layers, dropout):
        ...
        ### Attention layer
        self.attn = MultiHeadAttention(self.attn_head, self.hidden_size, dropout=0.)
        self.attn2 = MultiHeadAttention(self.attn_head, self.hidden_size, dropout=0.)
        self.add_pe = PositionalEncoding(self.hidden_size, 0.)

        ### Belief Tracker
        self.nbt = Encoder(EncoderLayer(self.hidden_size,
                                        MultiHeadAttention(self.attn_head, self.hidden_size, dropout=0.),
                                        PositionwiseFeedForward(self.hidden_size, self.hidden_dim, 0.),
                                        dropout),
                                        N=num_layers)
        self.decoder = Decoder(DecoderLayer(self.hidden_size,
                                        MultiHeadAttention(self.attn_head, self.hidden_size, dropout=0.),
                                        PositionwiseFeedForward(self.hidden_size, self.hidden_dim, 0.),
                                        dropout),
                                        N=num_layers)
    ...
    def forward(self, input_ids, result_masks, memory=None, is_mask=False):
        ...
        if not is_mask:
            ...
            memory = self.add_pe(memory)
            hidden = self.decoder(hidden, turn_mask, memory)
